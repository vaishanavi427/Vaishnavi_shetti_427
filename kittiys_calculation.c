...
typedef struct NodeQueryInfo {	// Query-specific node metadata
    SumType diqSum;		//   sum of node vals of all descendants in the queryset
    SumType partial;		//   sum of (d.val * dist(d, node)) for all descendants d of node (including node) that are in queryset
    SumType sumFrom;		//   total "Kitty's calc" sum from subtree rooted at this node
} NodeQueryInfo;
...
typedef struct Tree {
    ...
    Node *restrict nodes;
    // These are reset every query
    NodeSet *restrict enqueued;
    NodeType *restrict queued;           // indexed by tree depth 0 ... maxDepth (inclusive)
...
static Tree *readTree(int n, FILE *reader) {
    ...
    for (int i = 0; i < n-1; i++) {
        ...
        if (a == 1 || tree->nodes[a-1].depth > 0) {
            ...
            /* assert(b == 1 || tree->nodes[b-1].depth > 0); */
            addChildTo(tree, b, a);
	}
    }
...
// Read a new query from an open file
...
// Return "Kitty's Calculation" with the given graph and nodeset
...
int main(int argc, char **argv) {
    // TODO: Take these on command line
